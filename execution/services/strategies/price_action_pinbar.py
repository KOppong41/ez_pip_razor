...
def run_price_action_pinbar(symbol: str, candles: List[Candle], cfg: Optional[PinBarConfig] = None) -> EngineDecision:
    ...
    if len(candles) < max(cfg.ema_period + 5, cfg.lookback_for_levels + 5):
        return EngineDecision(action="skip", reason="not_enough_candles", strategy="price_action_pinbar")
    ...
    if pin_type is None:
        return EngineDecision(action="skip", reason="no_pinbar", strategy="price_action_pinbar")

    if not _trend_ok(pin_type, ema, idx, last["close"]):
        return EngineDecision(action="skip", reason="trend_filter_fail", strategy="price_action_pinbar")

    levels = _collect_wick_levels(candles[:-1], cfg)
    if not _pin_has_valid_level(pin_type, last, levels, cfg):
        return EngineDecision(action="skip", reason="no_sr_confluence", strategy="price_action_pinbar")
