...
def recent_signals(limit: int = 10):
    """
    Fetch the most recent signals for display on the admin dashboard.
    """
    try:
        Signal = apps.get_model("execution", "Signal")
        return (
            Signal.objects.select_related("bot")
...
def recent_bots(limit: int = 20):
    ...
    try:
        ...
        return (
            Bot.objects.select_related("asset", "broker_account", "owner")
...
def recent_broker_accounts(limit: int = 20):
    ...
    try:
        ...
        return (
            BrokerAccount.objects.select_related("owner")
            .order_by("-created_at")[:limit]
        )
    except Exception:
        return []
...
def recent_decisions(limit: int = 25):
    """
    Fetch the most recent decisions for custom admin pages.
    """
    try:
        Decision = apps.get_model("execution", "Decision")
        return (
            Decision.objects.select_related("bot", "signal")
...
def positions_long_short():
    """
    Return a dict with counts of long vs short positions (by qty sign).
    Assumes qty > 0 is long (buy), qty < 0 is short (sell).
    """
    try:
        Position = apps.get_model("execution", "Position")
...
def recent_orders(limit: int = 25):
    ...
    try:
        ...
        return (
            Order.objects.select_related("bot", "broker_account")
            .order_by("-created_at")[:limit]
        )
    except Exception:
        return []
...
def recent_trade_logs(limit: int = 100):
    ...
    try:
        ...
        return (
            TradeLog.objects.select_related("order", "bot", "broker_account")
            .order_by("-created_at")[:limit]
        )
    except Exception:
        return []
...
def recent_positions(limit: int = 25):
    ...
    try:
        Position = apps.get_model("execution", "Position")
        return (
            Position.objects.select_related("broker_account")
